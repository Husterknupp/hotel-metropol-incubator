...
- **Locking**: Emoji-Reaktion (`eyes`).
  ...
  - Zweiter paralleler Lauf findet Reaktion → bricht ab. Bei echtem Fehler: Reaktion(en) entfernen → nächster Cron-Lauf verarbeitet erneut.
- **Outcome-Reaktionen (Issue #7, korrigiert in PR #16)**: 👀 bleibt das Pending-Signal — es gibt in der GitHub-Reactions-API keinen Sanduhr/Pending-Content (nur `+1`, `-1`, `laugh`, `confused`, `heart`, `hooray`, `rocket`, `eyes`), daher wird 👀 nie entfernt, sondern bei Erfolg um `rocket` (🚀, Default `SUCCESS_REACTION`), bei einem echten Fehler um `confused` (😕, Default `ERROR_REACTION`) und beim eigenen Timeout um `+1` (👍, Default `TIMEOUT_REACTION`) ergänzt — alle Reaktionstypen sind auf GitHub unabhängig, ein Kommentar kann mehrere gleichzeitig tragen.
  **Warum `TIMEOUT_REACTION` (Issue #6-Folgekommentar, PR #16-Review):** ohne sie sahen zwei ganz unterschiedliche Zustände identisch aus — ein bare 👀 bedeutete sowohl "unser eigener 11min-Timeout griff, Turn vermutlich noch gesund" als auch "der Prozess ist irgendwann still gestorben, bevor überhaupt ein Ausgang erreicht wurde" (z.B. Absturz zwischen Lock-Erwerb und `sendEvent`-Rückkehr). 👀+👍 markiert jetzt explizit den ersten Fall; ein bare 👀 ohne jede weitere Reaktion, das über einen Poll-Zyklus hinaus bestehen bleibt, zeigt jetzt spezifisch den zweiten, schwerer zu diagnostizierenden Fall an. Kein vollständiger Stale-Lock-Detektor (nichts prüft aktiv das Lock-Alter) — nur eine Möglichkeit, die beiden Zustände anhand der Reaktionen zu unterscheiden. `sendEvent`s eigener `execSync`-Timeout wurde von 5min auf 11min angehoben (über der CLI-eigenen 600s/10min-Turn-Grenze), und `err.code === "ETIMEDOUT"` unterscheidet "unser Timeout griff, Turn läuft vermutlich noch" (Lock bleibt unangetastet, kein Fehler, kein Retry) von einem echten CLI-Fehler. Live-Beleg für die Notwendigkeit des höheren Timeouts: PR #62 auf `party-insights-shenanigans`, 2026-07-23 — `execSync` warf `ETIMEDOUT` nach 5min, der Turn lief serverseitig weiter und schloss 36s später erfolgreich ab (Commit `9fa6e1e`), das alte Verhalten hätte das fälschlich als Fehler gemeldet und den Lock unnötig released.
  **Korrektur (Husterknupps Review auf PR #16, 2026-07-23):** Ein echter (nicht-Timeout-)Fehler released den Lock jetzt NICHT mehr. Die ursprüngliche Fassung tat das ("nächster Cron-Lauf retriedet"), aber echte Fehler (falsche CLI-Argumente, aufgebrauchtes Provider-Kontingent, kaputte Config) beheben sich nicht von selbst — ein Retry hätte exakt die 2026-07-20/21-Schleife wiederhergestellt, die dieses ganze Vorhaben eigentlich verhindern soll (siehe #7/#8). Ein echter Fehler hinterlässt jetzt 👀+😕 gemeinsam und bleibt sichtbar hängen, bis ein Mensch 👀 manuell entfernt (Behebung + neuer manueller Kommentar/Retrigger).
  **Zweite Korrektur (Husterknupps Review auf PR #16, 2026-07-23):** Die Notification wird jetzt in allen drei Fällen (Erfolg, echter Fehler, eigener Timeout) als gelesen markiert. Der Lock allein verhindert bereits jede weitere Verarbeitung — ein späterer Poll auf einer ungelesenen Notification hätte ohnehin nur "already locked" geloggt und nichts weiter getan. Ungelesen zu lassen hätte also kein zusätzliches Signal erhalten, nur wiederholte, unnötige API-Aufrufe verursacht. Das sichtbare Signal (👍/😕/🚀) liegt ohnehin auf dem Kommentar/Issue selbst, unabhängig vom Gelesen-Status der Notification.
  **Fehler-Taxonomie (was wir konkret erwarten):**
  - **Sofort (Sekundenbruchteile, `<10s`):** CLI-Argument-Fehler (z.B. das defekte `--message`, das die 2026-07-20/21-Schleife auslöste — 1291 Log-Zeilen, jede binnen Sekunden gescheitert) — scheitert beim Argument-Parsing, bevor überhaupt ein Modell-Call losgeht. Strukturell schnell, kein Zufall.
  - **≥11min (unser `execSync`-Timeout, `ETIMEDOUT`):** wird NICHT als Fehler behandelt, sondern als "vermutlich noch laufend" — 👀 bleibt, kein 😕, kein Release. Empirisch bislang: alle 38 bisher geloggten `ETIMEDOUT`-Fälle waren gesunde, nur zu langsame Turns (nie ein echter, langsam eintretender Fehler beobachtet).
  - **Dazwischen (Sekunden bis 11min):** kein einziger Fall dieser Art bisher in unseren Logs beobachtet — insbesondere kein Beleg dafür, dass ein aufgebrauchtes Provider-Kontingent langsam statt sofort scheitert. Das ist eine offene Lücke in unserer Evidenz (unser altes 5min-Timeout hat dieses Fenster nie sauber beobachten lassen), keine bestätigte Aussage, dass es diesen Fall nicht gibt. Träfe ein solcher Fall ein, würde er heute als "echter Fehler" behandelt (😕, kein Release) — das ist die sichere Default-Annahme für einen unbeobachteten Fall.
...
| `LOCK_REACTION` | `eyes` | Emoji-Reaktion als verteiltes Lock, verdoppelt als Pending-Signal (Issue #7 — s.o.) |
| `SUCCESS_REACTION` | `rocket` | Zusätzliche Emoji-Reaktion bei Erfolg (auf 👀 aufgesetzt, nicht ersetzt) |
| `ERROR_REACTION` | `confused` | Zusätzliche Emoji-Reaktion bei einem echten (nicht-Timeout-)Fehler |
| `TIMEOUT_REACTION` | `+1` | Zusätzliche Emoji-Reaktion, wenn `sendEvent` unseren eigenen 11min-`execSync`-Timeout trifft (`ETIMEDOUT`) — unterscheidet "wir wissen, dass unser Timeout griff" von einem seit Beginn aus unbekanntem Grund hängenden Lock (Issue #6, PR #16-Review) |
...
| `OPENCLAW_WARN_REPLY_CHANNEL` | `discord` | Ziel-Channel für die isolierte Warn-Session, da diese (anders als `agent:main:main`) kein eigenes gebundenes Ziel hat (Fund 2026-07-22) |
| `OPENCLAW_WARN_REPLY_TO` | _(nicht gesetzt — erforderlich)_ | Ziel-Empfänger für die isolierte Warn-Session, z.B. `user:<discord-user-id>` (gleiches Format wie `openclaw agent --reply-to`). Bewusst ohne Default: identifiziert eine konkrete Person, jedes Deployment muss seinen eigenen Wert setzen statt einen fest im Repo hinterlegten zu erben. |
...
- [#6](https://github.com/Husterknupp/hotel-metropol-incubator/issues/6) ✅ geschlossen (partiell erledigt, Husterknupp 2026-07-23): Sichtbares Fehler-Signal auf GitHub bei gescheitertem `openclaw agent`-Aufruf — durch die Outcome-Reaktionen aus #7/#16 abgedeckt (inkl. `TIMEOUT_REACTION`). Verbleibende Lücke (aktiver Stale-Lock-Detektor für einen still gestorbenen Prozess) bewusst nicht gebaut — als theoretisch eingestuft, da nie beobachtet; keine eigene Folge-Issue angelegt
...
- [x] **PR #16 Review — Thread wird auch bei Fehler/Timeout als gelesen markiert** (Fund + Fix 2026-07-23, Husterknupps Review): Husterknupp fragte, warum die Notification bei einem sofortigen Fehler nicht als gelesen markiert wird, da ein erneuter Poll ohnehin nur "already locked" ergibt. Zu Recht — beide Codepfade (`run()`, `handlePrReviewCommentBatch()`) markieren den Thread jetzt auch bei echtem Fehler und bei `ETIMEDOUT` als gelesen, nicht nur bei Erfolg. Kein Signalverlust: das sichtbare Ergebnis (👍/😕/🚀) liegt auf dem Kommentar/Issue selbst, unabhängig vom Gelesen-Status. 4 Tests angepasst, 80/80 grün.
- [x] **Issue #6 — `TIMEOUT_REACTION` (👍) als Unterscheidungssignal** (Fund + Fix 2026-07-23, PR #16-Review): Husterknupp wies darauf hin, dass ein bare 👀 zwei unterschiedliche Zustände nicht unterscheidbar macht — "unser eigener Timeout griff" vs. "Prozess seit Minute 1 still gestorben". Neue Reaktion `TIMEOUT_REACTION` (Default `+1`) wird jetzt beim `ETIMEDOUT`-Fall zusätzlich zu 👀 gesetzt, in beiden Codepfaden (`run()`, `handlePrReviewCommentBatch()`). Kein aktiver Stale-Lock-Detektor (kein Lock-Alter-Check) — nur ein Unterscheidungssignal für den Fall, der bislang beobachtbar ist. 2 Tests angepasst, 80/80 grün.
- [x] **PR #16 Review — echter Fehler released den Lock nicht mehr** (Fund + Fix 2026-07-23, Husterknupps Review): Die erste #7-Fassung released den Lock bei jedem echten (nicht-Timeout-)Fehler, damit "der nächste Cron-Lauf retriedet" — Husterknupp merkte zurecht an, dass genau das die 2026-07-20/21-Schleife (1291 Log-Zeilen, dieselbe Argument-Fehler binnen Minuten wiederholt) wiederherstellt, da diese Fehlerklasse sich nicht von selbst behebt. Beide Codepfade (`run()` Einzelpfad, `handlePrReviewCommentBatch()`) released den Lock bei echtem Fehler jetzt nicht mehr — 👀+😕 bleiben zusammen sichtbar hängen, bis ein Mensch eingreift. Fehler-Taxonomie (was binnen Sekunden vs. was erst bei 11min als Timeout gilt vs. die unbeobachtete Lücke dazwischen) dokumentiert oben unter "Outcome-Reaktionen". 2 Tests umbenannt/angepasst, 80/80 grün.
- [x] **Issue #7 — Outcome-Reaktionen + realistischer Timeout** (Fund + Fix 2026-07-23): `execSync`-Timeout in `sendEvent` von 300000ms (5min, enger als die CLI-eigene 600s-Turn-Grenze) auf 660000ms (11min) angehoben; `err.code === "ETIMEDOUT"` unterscheidet "unser Timeout griff" (Lock bleibt, kein Fehler) von einem echten CLI-Fehler (löst — Stand PR #16-Review — KEIN Lock-Release mehr aus, siehe Eintrag oben). Bei Erfolg wird zusätzlich `rocket` auf denselben Lock-Zielen gesetzt (👀 bleibt bestehen, GitHub-Reaktionen unterschiedlichen `content`s sind unabhängig — vermeidet das von Husterknupp benannte Risiko einer fälschlich gelöschten Reaktion). Kein Sanduhr/Pending-Reaction möglich (GitHub-API kennt nur `+1`/`-1`/`laugh`/`confused`/`heart`/`hooray`/`rocket`/`eyes`) — 👀 allein bleibt daher das Pending-Signal. Ursprüngliche AC „Cron-Tick blockiert nicht während des Agenten-Turns" (Fire-and-Forget) wurde in der Issue-Diskussion bewusst zurückgestellt (Option a statt b) — synchron bleiben, aber mit realistischem Timeout und echtem Signal, bis Blockieren selbst zum beobachteten Problem wird. 5 neue Regressionstests, 80/80 grün.
- [x] **Konfiguration über `.env` statt Repo-Defaults** (Fund + Fix 2026-07-22): Personenbezogene Werte (`OPENCLAW_WARN_REPLY_TO`, s.u.) dürfen nicht mehr als Default im Quellcode landen — der Vorfall unten zeigt warum. `index.js` lädt jetzt `.env` per `dotenv` (leise, `quiet: true`, da Cron minütlich läuft) noch vor den anderen `require()`s, damit modul-level `process.env`-Reads die Werte sehen. `.env` ist gitignored; `.env.example` (Platzhalter, sicher zu committen) dokumentiert die nötigen Variablen. `README.md` „Setup" entsprechend erweitert.
- [x] **Fehlendes Zustellungsziel der isolierten Warn-Session** (Fund + Fix 2026-07-22, im selben PR #15 nachgezogen): Husterknupp meldete, dass ein CodeRabbit-Review auf `party-insights-shenanigans#60` weder Lock-Emoji noch Chat-Warnung erzeugte. Log-Analyse zeigte: der Listener hat `coderabbitai[bot]` korrekt als untrusted erkannt (kein Fehlverhalten, keine Code-Änderung), aber `sendWarning()` schlug beide Male, seit die isolierte Session oben live ist, mit `GatewayClientRequestError: Discord recipient is required` fehl — 100% Fehlerquote. Ursache: `--deliver` auf `agent:main:main` braucht keine weiteren Flags, weil diese Session aus normalem Gebrauch bereits ein gebundenes Discord-Ziel hat; die neue, nie zuvor genutzte `gh-warnings`-Session hat kein solches Ziel. Fix: `sendWarning()` hängt jetzt immer ein explizites `--reply-channel`/`--reply-to` an (`OPENCLAW_WARN_REPLY_CHANNEL`/`OPENCLAW_WARN_REPLY_TO`). Das fehlende Lock-Emoji auf dem Review selbst ist dagegen keine Regression, sondern eine GitHub-API-Grenze: Reviews (im Gegensatz zu Issue-/Inline-Review-Kommentaren) haben gar keinen Reactions-Endpunkt.
