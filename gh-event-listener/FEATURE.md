...
- **Outcome-Reaktionen (Issue #7)**: 👀 bleibt das Pending-Signal — es gibt in der GitHub-Reactions-API keinen Sanduhr/Pending-Content (nur `+1`, `-1`, `laugh`, `confused`, `heart`, `hooray`, `rocket`, `eyes`), daher wird 👀 nie entfernt, sondern bei Erfolg um `rocket` (🚀, Default `SUCCESS_REACTION`) bzw. bei einem echten Fehler um `confused` (😕, Default `ERROR_REACTION`) ergänzt — beide Reaktionstypen sind auf GitHub unabhängig, ein Kommentar kann 👀 UND 🚀 gleichzeitig tragen. `sendEvent`s eigener `execSync`-Timeout wurde von 5min auf 11min angehoben (über der CLI-eigenen 600s/10min-Turn-Grenze), und `err.code === "ETIMEDOUT"` unterscheidet "unser Timeout griff, Turn läuft vermutlich noch" (Lock bleibt unangetastet, kein Fehler, kein Retry) von einem echten CLI-Fehler (Lock wird released, `confused` gesetzt, nächster Cron-Lauf retriedet). Live-Beleg für die Notwendigkeit: PR #62 auf `party-insights-shenanigans`, 2026-07-23 — `execSync` warf `ETIMEDOUT` nach 5min, der Turn lief serverseitig weiter und schloss 36s später erfolgreich ab (Commit `9fa6e1e`), das alte Verhalten hätte das fälschlich als Fehler gemeldet und den Lock unnötig released.
